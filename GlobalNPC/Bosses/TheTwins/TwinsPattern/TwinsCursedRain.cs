...
namespace YourModName.Content.NPCs
{
    ...
    public static class TwinsCursedRain
    {
        ...
        private const int CursedFlameDamage = 10; // GreenBolt: target ~30 DMG Master / 3 (engine auto-triples proyektil di Master mode) - +20% otomatis kalau Last Stand
        ...
        private const int CursedBeamDamage = 36; // Beam Predik: base 15 kena ~80 di game (rasio observasi ~5.33x, BUKAN 3x kayak dugaan awal) - dinaikin ke 36 buat target ~190
        ...
        public static void Start(TwinsReworkOverride self)
        {
            ...
            // (self.LastStandActive true), skip random & pakai MaxRepeatsInclusive langsung -
            // sama pola kayak TwinDash.DashRepeatsRemaining. Di luar Last Stand tetap random.
            self.CursedRainRepeatsRemaining = self.LastStandActive ? MaxRepeatsInclusive : Main.rand.Next(MinRepeats, MaxRepeatsInclusive + 1);
        }
        ...
        // Nembak GreenBolt dari "depan", ngikutin arah hadap LIVE si Twin (yang lagi
        ...
        //
        // CATATAN: pakai GreenBolt (custom ModProjectile), BUKAN CursedFlameHostile vanilla.
        // Konsekuensi: (1) TIDAK ada .GetGlobalProjectile<TwinsDebuffGlobalProjectile>().IsFromTwins
        // — GreenBolt tidak terdaftar di AppliesToEntity, manggil GetGlobalProjectile pada tipe
        // yang tidak terdaftar — KeyNotFoundException — crash/instant death.
        // (2) ai[0] = angle dan ai[1] = 0f WAJIB dipass, karena AI() GreenBolt membaca
        // ai[0] buat arah gerak dan ai[1] buat kurva speed exponential.
        // (3) Initial velocity = facingDir * GreenBolt.StartSpeed (bukan CursedFlameSpeed)
        // — GreenBolt override velocity ini tiap tick dari AI().
        // GreenBolt nge-handle debuffnya sendiri di OnHitPlayer() (Cursed Inferno).
        private static void FireCursedFlame(NPC npc)
        {
            ...
            Projectile.NewProjectile(
                ...
                ModContent.ProjectileType<GreenBolt>(),
                TwinsReworkOverride.ScaleBoltDamage(CursedFlameDamage),
                2f,
                Main.myPlayer,
        ...
        {
            // ==========================================
            // EFEK "LASER LEPAS TEMBAK" - COSTUME BeamShot (dulu Zombie104) + screen shake +
            // flashbang merah non-solid, PERSIS pola yang sama kayak TwinsRetBeam pas beam-nya
            // nembak. Sengaja dijalanin di SEMUA client (sebelum netMode check di bawah),
            // soalnya ini murni audio/visual - biar semua orang di sesi MP tetap kerasa
            // efeknya, walau proyektil beam-nya sendiri cuma di-spawn dari server.
            // ==========================================
