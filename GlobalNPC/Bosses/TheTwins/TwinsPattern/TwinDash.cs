...
namespace YourModName.Content.NPCs
{
    ...
    public static class TwinDash
    {
        ...
        // DashDuration/DashSpeed/EyeFireIntervalTicks di-"internal" (bukan private) supaya
        // TwinsEnrageSweep.cs bisa reuse angka PERSIS yang sama buat dash-nya sendiri
        // (versi Phase 1/non-enrage), tanpa duplikasi angka yang bisa ke-drift.
        internal const float DashDuration = 55f;          // diperpanjang biar jarak tempuh jauh
        internal const float DashSpeed = 34f;             // dipercepat, biar dash-nya nembus jauh ngelewatin player
        ...
        internal const int EyeFireIntervalTicks = 6;      // EyeFire di-spam tiap 0.1 detik
        ...
        private const int DashBoltDamage = 10; // RedBolt: target ~30 DMG Master / 3 (engine auto-triples proyektil di Master mode) - +20% otomatis kalau Last Stand (lihat TwinsReworkOverride.ScaleBoltDamage)
            ...
            switch (state)
            {
                case State.Hovering:
                    ...
                    if (timer >= HoverDuration)
                    {
                        ...
                        // FIX (request "Last Stand pakai jumlah repeat paling maks"): SELAMA
                        // Last Stand (self.LastStandActive true), skip random-nya total & pakai
                        // MaxDashRepeatsInclusive langsung - biar attack loop Last Stand SELALU
                        // dapet jatah repeat maksimal tiap pattern, bukan kena RNG rendah yang
                        // bikin durasinya kependekan. Di luar Last Stand (normal/Enraged biasa),
                        // tetap random 3-5x seperti biasa.
                        self.DashRepeatsRemaining = self.LastStandActive ? MaxDashRepeatsInclusive : Main.rand.Next(MinDashRepeats, MaxDashRepeatsInclusive + 1);
                ...
                case State.Telegraph:
                    ...
                    if (timer >= TelegraphDuration)
                    {
                        ...
                        // Suara "Roar" tiap dash dilepas — COSTUME (TwinsSounds.TwinRoar,
                        // pitch di-naikin biar cempreng), bukan lagi sound internal Terraria,
        ...
        // internal (bukan private) — dipanggil ulang dari TwinsEnrageSweep.cs buat step
        // dash versi Phase 1/non-enrage di pattern itu.
        internal static void FireEyeFire(NPC npc, Vector2 dashDirection)
        {
        ...
        private static void FireDashSideBolts(NPC npc, Vector2 dashDirection)
        {
            ...
            Projectile.NewProjectile(
                ...
                boltType,
                TwinsReworkOverride.ScaleBoltDamage(DashBoltDamage),
                1.5f,
                Main.myPlayer,
        ...
        // internal (bukan private) — dipanggil ulang dari TwinsEnrageSweep.cs buat step
        // dash versi Phase 1/non-enrage di pattern itu.
        internal static void FireDeathLaserVolley(NPC npc, TwinsReworkOverride self, Vector2 dashDirection)
        {
            // COSTUME: dulu volley 5x DeathLaser ini gak punya sound sendiri sama sekali -
            // sekarang dikasih NormalLaser, SEKALI per volley (bukan per pellet, biar gak
            // numpuk 5x barengan di tick yang sama). Dijalanin di semua client (sebelum
            // netMode check) biar tetap kedengeran walau proyektilnya cuma di-spawn server.
