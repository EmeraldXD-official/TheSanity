...
namespace TheSanity.Projectiles
{
    ...
    public class RedPhantasmalBolt : ModProjectile
    {
        ...
        {
            // ==========================================
            // COSTUME: TwinsSounds.RedBoltShot (dulu SoundID.Item72 vanilla) tiap kali bolt ini
            // KELUAR/MUNCUL - dicek dari ai[1] == 0 (tick PERTAMA sejak spawn, SEBELUM
            // counter-nya di-increment di bawah), jadi cuma bunyi SEKALI per proyektil, bukan
            // tiap tick selama dia hidup.
            // ==========================================
            if (Projectile.ai[1] == 0f)
            {
                SoundEngine.PlaySound(TwinsSounds.RedBoltShot, Projectile.Center);
            }
        ...
        // Damage RedBolt SENGAJA ignore SELURUH defense DAN damage reduction player (endurance,
        // Paladin's Shield, dsb) - lewat helper satu tempat di TwinsDebuffGlobalProjectile biar
        // konsisten sama semua sumber damage Twins lain.
        ...
        // (satu tempat, dipakai bareng semua sumber damage Twins yang lain), DITAMBAH Ichor -
        // request eksplisit, durasi-nya "biarin" pakai DebuffDurationTicks yang sama (satu
        // konstanta di TwinsDebuffGlobalProjectile, dipakai juga buat Ichor dari DeathLaser/
        // Spectre Retinazer), BUKAN durasi baru yang di-custom sendiri di sini.
        public override void OnHitPlayer(Player target, Player.HurtInfo info)
        {
            TwinsDebuffGlobalProjectile.ApplyDebuffs(target);
            target.AddBuff(BuffID.Ichor, TwinsDebuffGlobalProjectile.DebuffDurationTicks);
        }
