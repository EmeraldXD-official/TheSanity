...
namespace YourModName.Content.NPCs
{
    ...
    public static class TwinsRetBeam
    {
        ...
        public static void Start(NPC npc, TwinsReworkOverride self)
        {
            ...
            // Aim line keluar -> sound Zombie103 (sound id internal Terraria).
            ...
            switch (state)
            {
                ...
                case State.FullBeam:
                    ...
                    if (self.RetBeamTimer >= preFireDelay)
                    {
                        ...
                        {
                            SoundEngine.PlaySound(SoundID.Zombie104, npc.Center); // suara RetLaser keluar
                        ...
                        {
                            DamageBeam(npc, self.RetBeamAimDirection, target);

                            // ENRAGED: 2 beam sisi (W-shape) juga ngedamage, ngikutin arah tengah.
                            if (self.IsEnraged)
                            {
                                DamageBeam(npc, self.RetBeamAimDirection.RotatedBy(SideSpreadAngle), target);
                                DamageBeam(npc, self.RetBeamAimDirection.RotatedBy(-SideSpreadAngle), target);
                            }
                        }
                    }
                ...
                case State.Reposition:
                    ...
                    if (distanceToTarget <= RepositionArriveThreshold || self.RetBeamTimer >= RepositionMaxDuration)
                    {
                        ...
                        // Aim line keluar lagi buat repeat berikutnya -> Zombie103 lagi.
                        SoundEngine.PlaySound(SoundID.Zombie103, npc.Center);
                    }
                    break;
        ...
        // Cek jarak player ke garis beam (segment dari npc.Center sepanjang BeamLength searah
        // direction). Kalau di bawah BeamHitThickness, kena damage.
        private static void DamageBeam(NPC npc, Vector2 direction, Player target)
        {
            if (!target.active || target.dead)
                return;

            Vector2 segStart = npc.Center;
