...
namespace TheSanity
{
    public class GoblinScoutRework : global::Terraria.ModLoader.GlobalNPC
    {
        ...
        {
            if (npc.type != NPCID.GoblinScout) return;
            ...
            if (npc.velocity.Y < 0 && npc.velocity.Y > -2f && !hasFiredInAir)
            {
                ...
                // --- LOKASI DAMAGE: 10 ---
                int p = Projectile.NewProjectile(npc.GetSource_FromAI(), npc.Center, shootVel, ProjectileID.ThrowingKnife, 5, 1f, Main.myPlayer);
